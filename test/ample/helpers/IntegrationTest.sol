...
import {AmpleEarn, VRFConfig} from "../../../src/ample/AmpleEarn.sol";

uint256 constant TIMELOCK = 1 weeks;

contract IntegrationTest is BaseTest {
    IAmpleEarnFactory ampleEarnFactory;
    IAmpleEarn internal vault;

    function setUp() public virtual override {
        super.setUp();

        ampleEarnFactory = new AmpleEarnFactory(admin, address(evc), address(permit2), address(perspective));
    ...
    // Deploy AmpleEarn from artifacts
    // Replaces using `new AmpleEarn` which would force 0.8.26 on all tests
    ...
    ) public returns (IAmpleEarn) {
        return IAmpleEarn(
            deployCode("AmpleEarn.sol", abi.encode(owner, evc, permit2, initialTimelock, asset, name, symbol))
        );
    }
    ...
    function _setTimelock(uint256 newTimelock) internal {
        ...
        (uint136 pendingTimelockValue, uint64 pendingTimelockValidAt) = vault.pendingTimelock();
        if (pendingTimelockValidAt == 0 || newTimelock != pendingTimelockValue) {
            vm.prank(OWNER);
    ...
    function _setGuardian(address newGuardian) internal {
        ...
        (address pendingGuardianValue, uint64 pendingGuardianValidAt) = vault.pendingGuardian();
        if (pendingGuardianValidAt == 0 || newGuardian != pendingGuardianValue) {
            vm.prank(OWNER);
    ...
    function _setCap(IERC4626 id, uint256 newCap) internal {
        _setCap(IEulerEarn(address(vault)), id, newCap);
    }
